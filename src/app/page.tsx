...
import { siteInfo as localInfo, ministries as localMinistries, homeEvents as localEvents, infoCards as localCards } from '@/lib/data'
import { getEvents, getMinistries, getSiteInfo, getInfoCards } from '@/lib/contentful'
import MinistryCard from '@/components/MinistryCard'

export default async function Home() {
  const [cmsSiteInfo, cmsMinistries, cmsEvents, cmsCards] = await Promise.all([
  ...
  const infoCards = cmsCards.length > 0 ? cmsCards : localCards
  return (
    <>
      <section id="hero" className="relative text-white py-24 bg-cover bg-center animate-fadeIn" style={{ backgroundImage: `url(${siteInfo.heroImage})` }}>
          ...
          <div className="grid md:grid-cols-3 gap-8 text-center">
            {infoCards.map((card, i) => (
              <div key={i}>
                ...
                <p className="text-sm text-gray-600">{card.text.split('\n').map((line, j) => <span key={j}>{line}<br /></span>)}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            groups[groupIndex].push(
              <div key={i} className="bg-white rounded-lg shadow-md overflow-hidden">
                <Image
                  ...
                  width={400}
                  height={300}
                  className="lazy-bg w-full h-48 object-cover"
                />
                <div className="p-4">
